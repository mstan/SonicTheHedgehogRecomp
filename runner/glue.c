...
/* Pacing mode (see glue.h).  Default is FIBER_FULL — current behavior. */
...
/* Fire the VBla handler once.  Caller ensures s_in_vblank_service is clear
 * and (in accurate mode) the per-wall-frame latch is honored.  Does NOT
 * touch g_cycle_accumulator — caller manages the accumulator per mode. */
static void fire_vblank_handler_once(void)
{
    ...
    int imask = (g_cpu.SR >> 8) & 7;
    if (imask >= 6)
        return;  /* interrupts masked — handler suppressed */
...
void glue_end_of_wall_frame(void)
{
    if (g_pacing_mode == GLUE_PACING_CYCLE_ACCURATE) {
         ...
         * one fire.  Hardware fires VBla every wall frame regardless
         * of what the 68K is doing. */
        if (!s_vblank_fired_this_frame) {
            if (g_cycle_accumulator < g_vblank_threshold)
                g_cycle_accumulator = g_vblank_threshold;
            glue_check_vblank();
    ...
    /* Reset the fired flag for the next wall frame.  In FIBER_FULL this
     * is also reset by glue_yield_for_vblank; resetting here is
     * idempotent and covers non-yielding wall frames. */
    s_vblank_fired_this_frame = 0;
}
